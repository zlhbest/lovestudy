...
public class Verification extends AppCompatActivity implements View.OnClickListener {
                ...
                case 0x00:
                    ...
                    if (result == SMSSDK.RESULT_COMPLETE) { //回调  当返回的结果是complete
                        if (event == SMSSDK.EVENT_GET_VERIFICATION_CODE) { //获取验证码
                            Toast.makeText(Verification.this, "发送验证码成功", Toast.LENGTH_SHORT).show();
                            Log.d(TAG, "get verification code successful.");
                        } else if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) { //提交验证码
                            Log.d(TAG, "submit code successful");
                            Toast.makeText(Verification.this, "提交验证码成功", Toast.LENGTH_SHORT).show();
                        ...
                        try {
                            ...
                            if (status > 0 && !TextUtils.isEmpty(des)) {
                                Toast.makeText(Verification.this, des, Toast.LENGTH_SHORT).show();
                                return;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
    ...
    private void login(){
        ...
        formBuilder.add("username",strPhoneNumber);//请求参数一      //该参数为手机号，用于传参
        formBuilder.add("password","123");//请求参数二
        RequestBody requestBody = formBuilder.build();

        Request.Builder builder = new Request.Builder().url("http://idashuai.cf/LoveStudy/Login").post(requestBody);
        execute(builder);
    }
    ...
    private void saveUserName(){
        ...
        //保存用户名
        editor.putString("username", strPhoneNumber);
        editor.apply();
